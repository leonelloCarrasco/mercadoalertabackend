...
const MODOS_VALIDOS = ['codigo', 'estado_fecha', 'proveedor', 'organismo'];
const RUT_REGEX = /^\d{1,2}(\.\d{3}){2}-[\dkK]$/;

/**
 * Cada modo de Licitaciones solo llena las columnas que le corresponden — ver
 * migración 033. Acá se valida que venga lo mínimo obligatorio por modo, sin
 * inventar defaults salvo los que la propia API ya documenta (estado="todos",
 * fecha=hoy — ese último se resuelve recién al EJECUTAR, no al guardar).
 */
function validarCampos(body) {
  ...
  if (tipo === 'licitacion') {
    ...
    const { montoMinimo, montoMaximo } = body;
...
router.post('/', async (req, res) => {
  const {
    ...
    montoMinimo, montoMaximo, regiones, organismos: organismosNombres,
  } = req.body;
  ...
  try {
    ...
    // a Compra Ágil o al modo 'organismo' de Licitaciones.
    const organismos = (tipo === 'compra_agil' || modo === 'organismo')
    ...
    const busqueda = await crearBusqueda(req.userId, {
      nombre: nombre.trim(),
      tipo,
      modo: tipo === 'licitacion' ? modo : null,
      codigoExterno: modo === 'codigo' ? codigoExterno.trim() : null,
      estado: modo === 'estado_fecha' ? (estado || 'todos') : null,
      fecha: (modo === 'estado_fecha' || modo === 'proveedor' || modo === 'organismo') ? (fecha || null) : null,
      rutProveedor: modo === 'proveedor' ? rutProveedor : null,
      montoMinimo: tipo === 'compra_agil' ? montoMinimo : null,
      montoMaximo: tipo === 'compra_agil' ? montoMaximo : null,
      regiones: tipo === 'compra_agil' ? regiones : null,
      organismos,
    });
...
// que corresponda (ver busqueda-ejecutor.service.js).
router.post('/:id/ejecutar', async (req, res) => {
  try {
    ...
    const resultado = await ejecutarBusqueda(busqueda);
